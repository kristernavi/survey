...
@section('content')
 <section class="section main">
    ...
    {{ Form::open(array('method' => 'PUT', 'id' => 'add-household-form' , 'url' => route('dog.update', $survey->id)  )) }}
...
@if(!$errors->any())
        <tr>
          @for ($i = 0; $i < count($survey->dogs); $i++)

      <td > <div class="field">

  <div class="control">
    <input class="input is-small" type="text" name="name[]" value="{{$survey->dogs[$i]->name}}">
  </div>
</div>
</td>
      <td >
  ...
  <div class="control">
    <div class="select is-small">
      {{ Form::select('origin[]',['Local' => 'Local', 'Out-sider' => 'Out-sider'], $survey->dogs[$i]->origin) }}
  ...
  <div class="control">
    <div class="select is-small">
      {{ Form::select('breed[]',['Mongrel Native' => 'Mongrel Native', 'Exotic Specify' => 'Exotic Specify'], $survey->dogs[$i]->breed) }}
  ...
  <div class="control">
    <input class="input is-small is-narrow" type="text" name="color[]" value={{ $survey->dogs[$i]->color}}>
  </div>
</div>
      </td>
 <td>
  ...
  <div class="control">
    <input class="input is-small is-narrow" type="month" name="age[]" value="{{ date('Y-m', strtotime($survey->dogs[$i]->age))}}">
  </div>
</div>
      </td>
       <td>
  ...
  <div class="control">
    <div class="select is-small">
      {{ Form::select('sex[]',['M' => 'M', 'F' => 'F'],$survey->dogs[$i]->sex) }}
    </div>
  </div>
</div>
      </td>
<td>
  ...
  <div class="control">
    <div class="select is-small">
      ...
      {{ Form::select('neutering[]',['Castrated' => 'Castrated', 'Intact' => 'Intact', 'Spayed' => 'Spayed'], $survey->dogs[$i]->neutering) }}
    </div>
  </div>
</div>
      </td>
